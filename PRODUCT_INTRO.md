...
当前，我们的测试工具链平台已支持七种类型的造数功能，具体包括：随机数造数、历史数据造数、历史规则造数、AI-表结构造数、AI-SQL脚本造数、AI-Mapping文档造数。其中，**AI-SQL脚本造数**功能的实现方式为：将测试所用的加工脚本 SQL 提供给 AI，由 AI 解析关联字段与脚本中使用的字段，进而开展关联造数与业务字段造数工作，以此辅助数据加工测试人员完成造数任务，提升测试质量与效率。

### 1.2 痛点

| 编号 | 痛点 | 具体表现 |
|------|------|---------|
| **1** | **AI-SQL 脚本造数存在天花板** | 当 SQL 脚本内容过大时，易超出 AI 调用的 token 限制，导致造数失败；当 SQL 脚本的关联关系较为复杂时，AI 对关联关系的解析可能出现错误，最终致使造数失败 |
| **2** | **多种造数方式相互孤立，无法融合** | 平台上的多种造数方式各有其适用场景（AI-SQL 脚本造数适用于包含关联字段的 SQL 脚本场景；AI-表结构造数适用于无关联字段的场景；历史规则造数适用于存在历史造数规则的场景等），但测试人员的造数场景复杂多样，并非局限于单一类型，而是需要融合多种场景进行造数 |
| **3** | **规则配置工作量大，且易与预期不符** | 当前造数平台要求用户为每张数据表的每个字段填写造数规则，多张表造数时工作量大，且可能因对造数规则的理解存在差异，导致生成的数据与用户预期不符 |

---

## 二、我们做了什么

**基于 AI 大模型的智能测试数据生成平台**：TestDataGen是一个基于AI的智能测试数据生成工具，核心目标是解决传统测试数据生成工具存在的痛点，通过结合人工智能大模型能力和数据库元数据解析技术，自动生成符合业务规则和数据库约束的高质量测试数据。
...
```
对SQL脚本进行拆分解析，提取脚本中的关联字段、WHERE条件等关键信息，通过代码实现关联字段的造数操作，确保关联字段造数成功率达到100%。

最后，基于“人机结合”理念，智能测试数据生成工具对重要功能进行拆分，确保重要字段造数成功率达100%；对于其他造数功能，则交由AI处理并设置兜底逻辑，保障测试数据生成成功。
...
效果：对于标准业务字段，知识库将"模型可能出错"转变为"规则保证正确"，用确定性替代概率性。

**2. 提示词按字段维度精准限制：缩小幻觉空间**

对于必须调用大模型的语义类字段（姓名、地址、公司名、备注等），平台通过将 Prompt 收敛到**单字段维度**，最大限度压缩模型的"自由发挥空间"，使其难以产生幻觉：

- **约束显式化**：每次 Prompt 仅包含单个字段的完整约束信息（数据类型、最大长度、可空性、语义描述），模型需要"记住"和"遵守"的规则极少，遵从率极高
- **输出格式固定**：要求模型严格返回 JSON 数组，格式简单明确，模型不会在格式上产生幻觉
- **长度硬约束**：Prompt 中明确标注 `maxLength`，即使模型偶尔超长，后置截断兜底确保数据不会因幻觉溢出
- **上下文隔离**：不同字段的 Prompt 互相独立，避免模型因上下文过长而"记混"约束、产生字段间串扰

对比：整表级 Prompt 中模型需同时满足 10+ 列约束，注意力分散导致幻觉频发；字段级 Prompt 中模型仅需关注 1 个字段的 1 组约束，认知负荷极低，幻觉空间被压缩到最小。
